...
import {
  ...
  breedingStatusStringToEnum,
  getBreedingStatusColor,
} from "../types/BreedingStatus";
...
const RabbitsPage = () => {
  ...
  const [addRabbitError, setAddRabbitError] = React.useState<string | null>(
    null
  );

  const [birthModalOpen, setBirthModalOpen] = React.useState(false);
  const [birthModalRabbit, setBirthModalRabbit] =
    React.useState<RabbitData | null>(null);
  const [birthModalError, setBirthModalError] = React.useState<string | null>(
    null
  );
  ...
  const apiFilters = React.useMemo(() => {
    ...
    if (
      filters.status &&
      breedingStatusStringToEnum[filters.status] !== undefined
    ) {
  ...
  const { rabbits, loading, error, totalCount, refetch } = useRabbitData({
    pageIndex: page,
  ...
  const handleRegisterBirth = async (offspringCount: number) => {
    ...
    setBirthModalError(null);
    try {
      await RabbitService.registerBirth(
        birthModalRabbit.rabbitId,
        offspringCount
      );
      setBirthModalOpen(false);
      await refetch();
        ...
        <>
          ...
          >
            ...
            >
              ...
              >
                <Typography variant="h5">Breeding Rabbits</Typography>
                      ...
                      <Grid size={{ xs: 12, sm: 6 }} key={rabbit.rabbitId}>
                        <Paper
                          sx={{
                            p: 2,
                            height: "100%",
                            cursor: isRabbitClickable(rabbit)
                              ? "pointer"
                              : "default",
